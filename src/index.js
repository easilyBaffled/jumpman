...
import level3 from "./levels/level3.txt";
import level4 from "./levels/level4.txt";
var LEVELS = [level1, level2, level3, level4].map((levelString) =>
  levelString.split("\n")
);
...
function element(name, className, id) {
...
function DOMDisplay(parent, level) {
  ...
  this.debug = parent.appendChild(element("div", undefined, "debug"));
  this.level = level;
...
DOMDisplay.prototype.drawFrame = function () {
  ...
  // const player = this.level.actors.find((a) => a.type === "player");
  // this.debug.innerText =
  //   `(${player.pos.x.toFixed(1)}, ${player.pos.y.toFixed(1)}) - ${
  //     player.speed.x
  //   }, ${player.speed.y.toFixed(2)}\n` + (this.debug.innerText || "");
...
var jumpSpeed = 17;
const maxKeyHold = 20;
const normalizeJumpPress = (v) => normalize(v, 0, maxKeyHold);
const jumpEasing = BezierEasing(0.22, 1, 0.36, 1);

Player.prototype.moveY = function (step, level, keys) {
  // if (this.jumping && keys.up && this.jumping < maxKeyHold) {
  //   this.jumping += 1;
  //   console.log(
  //     this.jumping,
  //     normalizeJumpPress(this.jumping),
  //     jumpEasing(normalizeJumpPress(this.jumping))
  //   );
  //   this.speed.y -= jumpEasing(normalizeJumpPress(this.jumping));
  // } else if (this.jumping && (!keys.up || this.jumping >= maxKeyHold)) {
  //   this.jumping = 0;
  // }
  // if (!this.jumping) {
  ...
  if (keys.down) {
    this.speed.y += step * gravity * 5;
  }
  var newPos = this.pos.plus(motion);
  var obstacle = level.obstacleAt(newPos, this.size);
  // console.log(this.jumping, keys.up);
...
function trackKeys(codes) {
  ...
  function handler(event) {
    if (codes[event.key]) {
      pressed[codes[event.key]] =
        event.type === "keydown" ? pressed[codes[event.key]] + 1 : 0;
...
// // popup
// document
//   .getElementsByClassName("pop-close")
//   .addEventListener("click", (elem) => {
//     elem.target.closest(".overlay").removeClass("show");
//   });
